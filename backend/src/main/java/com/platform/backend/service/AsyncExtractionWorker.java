...
// Holds the actual extraction work as its own bean, separate from
// ExtractionService, specifically so @Async takes effect. Spring's
// annotation-based async/proxy support does not apply to a method calling
// another method on itself within the same class (self-invocation bypasses
// the proxy) — it only works when one bean calls a method on a different
// bean, which is why this logic had to move out of ExtractionService.
@Service
@RequiredArgsConstructor
public class AsyncExtractionWorker {
    ...
    @Async
    public void runExtractionAsync(String documentId) {
        try {
            doExtract(documentId);
        } catch (Exception e) {
            documentRepository.findById(documentId).ifPresent(doc -> {
                doc.setStatus(DocumentStatus.EXTRACTION_FAILED);
                doc.setUpdatedAt(Instant.now());
                documentRepository.save(doc);
            });
            System.err.println("Extraction failed for document " + documentId + ": " + e.getMessage());
        }
    }

    private void doExtract(String documentId) throws Exception {
        Document document = documentRepository.findById(documentId)
                .orElseThrow(() -> new IllegalStateException("Document disappeared during extraction: " + documentId));
